...
import { History, CheckCircle2, Calendar, Film } from "lucide-react";
import { Card, CardHeader, CardTitle, CardDescription, CardContent } from "../ui/card";
import { Button } from "../ui/button";
import { useDashboard } from "../../context/DashboardContext";

export default function ProjectHistoryList({ onViewAll }) {
      ...
      <CardContent className="p-0 overflow-x-auto">
        {loading ? (
          ...
          <div className="p-10 text-center space-y-2">
            ...
            <p className="text-xs font-bold text-white">No past projects archived yet</p>
            <p className="text-[11px] text-slate-400">Completed projects will appear here once delivered.</p>
          </div>
        ) : (
                ...
                <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-mono mt-0.5">{item.currentVersion || "V1"} archived</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-4 px-6 text-slate-400 font-mono">
                    <span className="flex items-center gap-1">
                      <Calendar className="w-3 h-3 text-slate-500" /> {item.completionDate || "Delivered"}
                    </span>
                  </td>

                  <td className="py-4 px-6 font-mono font-bold text-emerald-400">
                    ${(item.finalRevenue || 0).toLocaleString()}
                  </td>
