...
export default function ProjectProgressView({ onViewAll }) {
      ...
      <CardContent className="p-6 space-y-4">
        {loading ? (
          <div className="space-y-4">
            {[1, 2].map((i) => (
              <div key={i} className="p-5 rounded-2xl bg-[#1A1B23]/70 border border-white/5 space-y-3 animate-pulse">
          ...
          <div className="p-10 text-center border border-dashed border-white/10 rounded-2xl space-y-3 bg-[#1A1B23]/30">
            <FolderKanban className="w-10 h-10 text-slate-600 mx-auto" />
            <p className="text-sm font-bold text-white">No active projects yet — create your first one</p>
            <p className="text-xs text-slate-400 max-w-sm mx-auto">
              Deploy your first frame-accurate review stream by clicking the "New Project" button.
            </p>
          </div>
        ) : (
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  >
                    ...
                    {project.status} (${(project.milestoneAmount || 0).toLocaleString()})
                  </span>
                </div>
              </div>
