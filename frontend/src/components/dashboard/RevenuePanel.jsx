...
import { DollarSign, Lock, Unlock, Clock, CheckCircle2, FileText } from "lucide-react";
import { Card, CardHeader, CardTitle, CardDescription, CardContent } from "../ui/card";
import { Button } from "../ui/button";
import { useDashboard } from "../../context/DashboardContext";

export default function RevenuePanel({ onViewAll }) {
  const { revenueList, stats, loading } = useDashboard();
      ...
      <CardHeader className="p-6 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/5">
        ...
        <div className="flex items-center gap-3">
          <div className="px-3 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs font-mono font-bold flex items-center gap-1.5">
            <Lock className="w-3.5 h-3.5" /> Pending: ${(stats.pendingRevenue || 0).toLocaleString()}
          </div>
          <div className="px-3 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-[#22C55E] text-xs font-mono font-bold flex items-center gap-1.5">
            <Unlock className="w-3.5 h-3.5" /> Paid: ${(stats.currentMonthRevenue || 0).toLocaleString()}
          </div>
          {onViewAll && (
      ...
      <CardContent className="p-0 overflow-x-auto">
        {loading ? (
          ...
          <div className="p-10 text-center space-y-2">
            ...
            <p className="text-xs font-bold text-white">No invoices or milestone locks recorded</p>
            <p className="text-[11px] text-slate-400">Invoices generated upon project creation will appear here.</p>
          </div>
        ) : (
